...
# Captain's Kit: resolve the auditor beside this sourced helper for standalone installs.
# Captain's Kit: measure rendered viewports, not unresolved gtable null units.

.nature_alignment_audit_script <- local({
  source_files <- vapply(sys.frames(), function(frame) {
    path <- frame$ofile
    if (is.character(path) && length(path) == 1L) path else NA_character_
  }, character(1))
  source_files <- source_files[!is.na(source_files)]
  if (!length(source_files)) {
    stop("Load panel_alignment.R with source() from its installed location", call. = FALSE)
  }
  helper <- normalizePath(tail(source_files, 1L), mustWork = TRUE)
  file.path(dirname(helper), "audit_panel_alignment.py")
})
...
write_patchwork_panel_layout <- function(
  ...
  panels <- lapply(seq_len(nrow(panel_rows)), function(index) {
    ...
    viewport <- sprintf("%s.%d-%d-%d-%d", row$name, row$t, row$r, row$b, row$l)
    ...
    # deviceLoc returns inches on the rendered device; the JSON auditor uses
    # PDF points (72/inch). A gtable's null units only resolve inside its viewport.
    bounds <- c(lower$x, lower$y, upper$x, upper$y) * 72
    grid::upViewport(0)
    list(
      id = panel_ids[index],
      bbox_pt = unname(bounds),
...
require_patchwork_panel_alignment <- function(
  ...
  column_groups = NULL,
  exemptions = list(),
  audit_script = .nature_alignment_audit_script,
