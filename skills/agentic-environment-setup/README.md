...
This Captain's Kit skill prepares or audits coding workspaces: project knowledge,
repeatable setup, code boundaries, verification, diagnostics and handoffs.
Invoke it explicitly for a target project. The agent follows [SKILL.md](SKILL.md)
and loads the relevant playbooks; this guide explains usage and examples.

The hypothesis is that better project foundations reduce interruptions, repeated
explanations and incorrect delegated changes. Verify that on your own projects.

[Quick start](#quick-start) · [Implementation examples](#key-implementations) ·
[Optional infrastructure](#optional-infrastructure) ·
[Audit example](#what-an-audit-looks-like) · [Installation](https://github.com/DrWenyuanHe/captains-kit#installation) ·
[Sources](#where-the-ideas-come-from)

![Manual invocation leads to relevant guidance, then setup implements and verifies or audit inspects and reports.](assets/diagrams/workflow.svg)

[Editable diagram source](assets/diagrams/workflow.mmd)
...
```text
Use https://github.com/DrWenyuanHe/captains-kit to set up this project for
...
```text
Use https://github.com/DrWenyuanHe/captains-kit to audit this project's
...
![AGENTS.md leads to the docs index, which points to the orders contract, canonical implementation and focused verification.](assets/diagrams/context-map.svg)

[Editable diagram source](assets/diagrams/context-map.mmd)
...
![A repeatable bootstrap prepares task configuration for the browser, server and CLI, while agent tools are checked independently against the intended development target.](assets/diagrams/environment-propagation.svg)

[Editable diagram source](assets/diagrams/environment-propagation.mmd)
...
![Task A and Task B have separate worktrees, preview ports and disposable state when their work requires backend isolation.](assets/diagrams/task-isolation.svg)

[Editable diagram source](assets/diagrams/task-isolation.mmd)
...
Follow the [environment-propagation procedure](references/environment-propagation.md)
and, for mutable services, the [state-isolation checklist](references/state-isolation.md).
...
Use the [task-record template](assets/templates/task-record.md)
and [evaluation guidance](references/evaluate.md).
The full [setup playbook](references/setup.md)
...
The [optional playbook](references/optional-infrastructure.md)
...
This kit's own [GitHub Actions workflow](https://github.com/DrWenyuanHe/captains-kit/blob/main/.github/workflows/check.yml) is a small
...
Read the [audit rubric](references/audit.md)
or use the [report template](assets/templates/audit-report.md).

## Invoke the installed skill

After following the [library installation instructions](https://github.com/DrWenyuanHe/captains-kit#installation),
invoke it in your target project:

- **Codex:** `$agentic-environment-setup setup this project`
- **Claude Code:** `/agentic-environment-setup setup this project`

Replace `setup` with `audit` to inspect the project without changing it.
The existing skill name and manual invocation policy are preserved.

## Skill resources

| Resource | Purpose |
| --- | --- |
| [Skill entrypoint](SKILL.md) | Select setup or audit and load relevant guidance. |
| [Setup](references/setup.md) and [audit](references/audit.md) playbooks | Detailed procedures and evidence criteria. |
| [Environment propagation](references/environment-propagation.md) | Prepare development profiles, preserve overrides and verify consumer targets. |
| [Optional infrastructure](references/optional-infrastructure.md) | Conditional CI/GitHub Actions and Docker/Supabase guidance. |
| [Templates](assets/templates/README.md) | Adaptable project map, task record and audit report. |
| [Source digests](references/sources/README.md) | Five attributed Markdown backups of the core ideas. |

The agent performs setup or audit by following the skill; this is not a codebase
audit scanner.
...
| --- | --- |
| [OpenAI: Harness engineering](https://openai.com/index/harness-engineering/) | [Repository foundations](references/sources/openai-harness-engineering.md) |
| [Anthropic: Effective context engineering](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) | [Focused context](references/sources/anthropic-context-engineering.md) |
| [Anthropic: Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents) | [Continuity across sessions](references/sources/anthropic-long-running-harnesses.md) |
| [Anthropic: Demystifying evals for AI agents](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents) | [Observable outcomes](references/sources/anthropic-agent-evals.md) |
| [Anthropic: Harness design for long-running apps](https://www.anthropic.com/engineering/harness-design-long-running-apps) | [Calibrating orchestration](references/sources/anthropic-harness-design.md) |
...
[evaluation guide](references/evaluate.md) to test
the hypothesis on your own work. [Field lessons](references/field-lessons.md)
